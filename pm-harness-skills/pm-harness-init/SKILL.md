...
description: 初始化 AI Coding 项目的 harness 工程结构。当用户需要为新项目生成标准化的 pm-harness 工程框架时使用本技能。触发场景包括：初始化新项目工程结构、生成 AI 编码脚手架、创建 pm-harness 工程、搭建 OpenSpec + AI Agent 规范编程项目结构。技能会按预设顺序收集产品信息、能力、形态、存储、技术栈、数据库、编码 Agent 与必填 UI 设计；上传 ui-design.md 时严格遵循，手工描述时自动推导设计方案，再生成完整的参数化 harness 工程 zip 包。
---
...
2. 严格按 schema 的 15 个输入步骤逐项收集或确认；用户一次性提供的有效字段可跳过重复提问，但不得改变剩余步骤顺序。
