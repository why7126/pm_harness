...
4. 创建模板中未预置的空目录：`issues/`、`iterations/change/`、`iterations/archive/`、`openspec/specs/`、`openspec/changes/`、`openspec/archive/`、`docs/knowledge-base/`、`data/`
...
│       └── archive/            # 已修复关闭并归档
├── iterations/
│   ├── change/                 # 未归档迭代，初始化阶段仅保留 .gitkeep
│   └── archive/                # 已归档迭代，初始化阶段仅保留 .gitkeep
├── openspec/
│   ├── project.md
│   ├── config.yaml
...
- 保持 AGENTS.md 与 README.md、project.yaml、rules/global.md、rules/directory-structure.md、rules/document-governance.md、rules/issues-lifecycle.md、rules/iterations-lifecycle.md、rules/requirement-management.md、rules/bug-management.md、rules/testing.md、rules/release.md 的流程、目录和命令命名一致。
...
- 根据 REQ_ROOT_DIR、BUG_ROOT_DIR、ITERATION_PATTERN、CHANGE_ID_PATTERN 生成 issues、iterations、openspec 的目录归属；必须与 issues-lifecycle.md、iterations-lifecycle.md、requirement-management.md、bug-management.md、document-governance.md 一致。
...
- 保持 directory-structure.md 与 AGENTS.md、README.md、project.yaml、global.md、document-governance.md、issues-lifecycle.md、iterations-lifecycle.md、requirement-management.md、bug-management.md、coding.md、testing.md、data-management.md、database.md 一致。
...
- 根据 ITERATION_PATTERN 与 SPRINT_FACT_SOURCE 生成迭代目录、四件套和事实源规则；未知时默认 `iterations/change/sprint-xxx/` 与 `sprint.yaml`，归档后移动到 `iterations/archive/sprint-xxx/`；必须与 iterations-lifecycle.md 一致。
...
- 保持 document-governance.md 与 AGENTS.md、global.md、directory-structure.md、issues-lifecycle.md、iterations-lifecycle.md、requirement-management.md、bug-management.md、api.md、database.md、testing.md、release.md、README.md 一致。
...
基于 `assets/pm-harness-template/rules/iterations-lifecycle.md` 的模块化框架生成，作为 Sprint 共用的 change、archive 两阶段物理目录生命周期规则。

生成要求：
- 保留所有 `[通用]` 模块，包括规则定位、文档模块分类、总原则、目录结构、阶段准入、迁移时机、sprint.yaml 字段、遗留路径兼容、与 Issues/OpenSpec/知识库的关系、自动化要求、初始化生成建议和检查清单。
- 用用户输入替换 `[个性化]` 占位符：`{PRODUCT_NAME}`、`{ITERATION_ROOT_DIR}`、`{ITERATION_PATTERN}`、`{SPRINT_FACT_SOURCE}`、`{ITERATION_LIFECYCLE_STAGES}`、`{SPRINT_STATUS_MACHINE}`、`{SPRINT_STATUS_TO_STAGE}`、`{SPRINT_COMMAND_SET}`、`{SPRINT_ARCHIVE_POLICY}`、`{SPRINT_REVIEW_POLICY}`、`{SPRINT_PATH_COMPAT_POLICY}`、`{WORKFLOW_SYNC_COMMAND}`、`{TASK_TRACKING_SYSTEM}`。
- 默认生成 `change`、`archive` 两阶段；如用户明确改名，必须同步 directory-structure、document-governance、AGENTS、Sprint 命令和校验脚本。
- 默认要求新建 Sprint 落入 `iterations/change/`，`/sprint-archive` 完成闭环后移动到 `iterations/archive/`。
- 根据是否启用 OpenSpec、Sprint、外部项目管理工具、遗留扁平路径和自动化同步脚本保留或删除 `[条件启用]` 模块；未启用能力不得保留不可执行的强制命令。
- `sprint.yaml` 字段必须至少能表达 `status` 与 `lifecycle_stage`；如启用外部看板字段、owner 字段或发布字段，必须说明同步策略。
- 保持 iterations-lifecycle.md 与 AGENTS.md、directory-structure.md、document-governance.md、sprint 命令、sync-workflow-status.py、docs/knowledge-base/sprints/ 一致。
...
□ rules/iterations-lifecycle.md 已生成，并与 Sprint/目录/文档治理规则一致
□ openspec/specs、changes、archive 仅保留 .gitkeep
□ iterations/change/ 与 iterations/archive/ 仅保留 .gitkeep
