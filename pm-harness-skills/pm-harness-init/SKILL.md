...
2. `references/agent-entrypoint.md`
...
3. 只保留 `.agents/skills/` 作为 Agent 技能入口；不得生成 `.claude/`、`.codex/`、`.cursor/`、`.kiro/`、`.opencode/` 等兼容工具目录。
4. 模板资产中的 `.agents/skills/*/SKILL.template.md` 在输出工程中必须渲染为 `.agents/skills/*/SKILL.md`；输出工程不得保留 `SKILL.template.md`。
5. 创建必要空目录并用 `.gitkeep` 保留。
6. 渲染模板中的产品名、项目编号、技术栈、命令、目录、服务、文档索引和规则入口。
...
□ 未启用端、服务、对象存储、算法已裁剪，且只保留 `.agents/skills/` 技能入口
