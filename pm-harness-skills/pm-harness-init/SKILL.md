...
初始化时采用“最小必填 + 推荐补充 + 默认推断”的方式收集信息。

- 用户只提供最小必填信息时，也必须能生成可用 Harness 工程。
- 未提供的信息必须写入 `待确认`、`enabled=false`、`planned` 或删除对应强制要求，不得编造。
- 用户后续提供更完整信息时，应优先用于替换各模板中的 `{...}` 占位符。
- 能力未启用时，应删除或弱化对应目录、文档、命令、Compose 服务、测试门禁和 OpenSpec 强制流程。

### 最小必填

| 参数 | 派生变量 | 说明 |
|---|---|---|
| 产品名称 | `PRODUCT_NAME` | 用于 README、AGENTS、docs、rules、project.yaml、OpenSpec 等文档标题和说明 |
| 项目代码 | `PRODUCT_CODE` | 可由产品名称派生为 kebab-case；用于目录名、容器名、网络名、包名、project.yaml |
| 产品简介 | `PRODUCT_DESCRIPTION` | 用于 README、docs/00-product-overview.md、openspec/project.md |
| 产品形态 | `PRODUCT_FORMS` | 至少一项：API 服务、Web 展示端、管理后台、H5、小程序、移动端、桌面端、SDK、算法/模型服务 |

### 推荐输入

| 参数 | 派生变量 | 默认/处理方式 | 影响范围 |
|---|---|---|---|
| 业务领域 | `BUSINESS_DOMAIN` | 未提供则 `待确认` | README、docs/00、project.yaml、AGENTS |
| 目标用户 | `TARGET_USERS` | 未提供则 `待确认` | README、docs/00、project.yaml |
| 核心能力 | `CORE_CAPABILITIES` | 未提供则 `待确认` | docs、OpenSpec、需求模板、测试映射 |
| 范围外能力 | `OUT_OF_SCOPE` | 未提供则 `待确认` | docs/00、openspec/project.md |
| 项目负责人 | `PROJECT_OWNER` / `PRODUCT_OWNER` | 未提供则 `待确认` | README、project.yaml、OpenSpec |
| 文档负责人 | `DOCS_OWNER` | 默认同项目负责人或 `待确认` | DOCUMENT_METADATA_INDEX、docs/README |
| 测试负责人 | `QA_OWNER` | 未提供则 `待确认` | testing rules、standards、OpenSpec 测试映射 |
| 项目生命周期 | `PROJECT_LIFECYCLE_STAGE` | `prototype` / `mvp` / `production` / `maintenance`，默认 `mvp` | project.yaml、release、testing |
| 仓库地址 | `REPOSITORY_URL` | 未提供则 `待确认` | project.yaml、README |

### 产品形态与能力开关

| 能力 | 变量 | 默认 | 影响范围 |
|---|---|---|---|
| 后端/API 服务 | `HAS_BACKEND`、`HAS_API_SERVICE`、`API_ENABLED` | 产品形态含 API 时 true | `src/backend/`、rules/api、docs/03、standards/api |
| Web 展示端 | `HAS_WEB_PUBLIC`、`HAS_WEB` | 产品形态含 Web 时 true | `src/web/`、ui-design、frontend-test |
| 管理后台 | `HAS_WEB_ADMIN` | 产品形态含管理后台时 true | docs/00、ui-design、auth、frontend-test |
| 小程序 | `HAS_MINIAPP` | false | `src/miniapp/`、compatibility/devices |
| 移动端 | `HAS_MOBILE`、`HAS_ANDROID`、`HAS_IOS` | false | `src/android/`、`src/ios/`、compatibility |
| 桌面端 | `HAS_DESKTOP` | false | `src/desktop/` 或等价目录、compatibility |
| SDK | `HAS_SDK` | false | `src/sdk/`、api-governance、compatibility |
| 算法/模型服务 | `HAS_ALGORITHM`、`AI_OR_ALGORITHM_ENABLED` | false | `src/algorithm/`、`models/`、docker-compose algorithm |
| 对象存储 | `OBJECT_STORAGE_ENABLED` | 有上传/媒体时默认 true，否则 false | rules/object-storage、docs/07、Compose object-storage |
| 媒体/上传 | `MEDIA_ENABLED`、`UPLOAD_ENABLED` | 有文件/图片/视频/导入时 true | rules/media、file_upload、docs/06 |
| 认证授权 | `AUTH_ENABLED` | 有用户/权限时 true | security、authentication、error-codes |
| 异步任务 | `ASYNC_TASK_ENABLED` | false | architecture、deployment、Compose worker |
| 缓存/搜索 | `CACHE_ENABLED`、`SEARCH_ENABLED` | false | architecture、deployment、compatibility |

### 技术栈输入

| 参数 | 变量 | 默认/处理方式 | 影响范围 |
|---|---|---|---|
| 后端语言 | `BACKEND_LANGUAGE` | 未提供则 `待确认` | coding、directory、architecture |
| 后端框架 | `BACKEND_FRAMEWORK` / `BACKEND_STACK` | 未提供则 `待确认` | AGENTS、README、rules、docs |
| 后端运行时版本 | `BACKEND_RUNTIME_VERSION` | 未提供则 `待确认` | environment、compatibility |
| 后端包管理器 | `BACKEND_PACKAGE_MANAGER` | 未提供则 `待确认` | README、scripts、testing |
| API 风格 | `API_STYLE` / `API_PROTOCOL` | 默认 REST，需标记可确认 | api rules、OpenAPI、standards |
| 前端框架 | `FRONTEND_FRAMEWORK` / `FRONTEND_STACK` | 无前端则 `不适用` | ui-design、frontend-test |
| 前端包管理器 | `FRONTEND_PACKAGE_MANAGER` | 未提供则 `待确认` | README、scripts |
| UI/组件库 | `STYLE_SYSTEM`、`COMPONENT_LIBRARY`、`ICON_LIBRARY` | 未提供则 `待确认` | ui-design、design-system |
| 数据库 | `DATABASE_STACK` / `DB_PRIMARY` | 未提供默认 `SQLite` 但标记可确认 | database、data-management、Compose database |
| 迁移策略 | `MIGRATION_STRATEGY` | 未提供则 `待确认` | database、release |
| 对象存储供应商 | `OBJECT_STORAGE_PROVIDER` / `OBJECT_STORAGE_STACK` | 未启用则 `不适用` | object-storage、media、Compose |
| 部署方式 | `DEPLOYMENT_STACK` | 默认 `docker-compose` | docs/02、docker-compose、deploy |
| 信创/兼容要求 | `XINCHUANG_REQUIREMENTS`、`OS_SUPPORT_MATRIX`、`DATABASE_SUPPORT_MATRIX` | 未提供则 `待确认` | compatibility、database、release |

### 可选输入文档

用户可以额外提供已有文档作为初始化事实源。提供后必须先解析文档内容，再写入对应 Harness 模块；不得只复制原文。

| 输入文档 | 派生变量 | 解析内容 | 自动同步目标 |
|---|---|---|---|
| `ui-design.md` | `UI_DESIGN_SOURCE_PATH`、`UI_DESIGN_SOURCE_CONTENT`、`UI_DESIGN_SOURCE_ENABLED` | 产品设计定位、目标用户、支持端、UI 技术栈、Design Token、组件体系、页面结构、交互状态、响应式、可访问性、主题/国际化、视觉验收、验证命令 | `rules/ui-design.md`、`AGENTS.md`、`README.md`、`project.yaml`、`docs/00-product-overview.md`、`docs/01-architecture.md`、`docs/05-compatibility-matrix.md`、`docs/standards/frontend-test-standard.md`、`docs/standards/test-coverage.md`、`DOCUMENT_METADATA_INDEX.md` |

`ui-design.md` 输入规则：
- 支持用户直接粘贴内容，或提供本地文件路径。
- 若用户提供路径，必须读取文件内容并提取结构化设计信息。
- `ui-design.md` 只作为事实源，不得把来源项目产品名、页面名、品牌、颜色、组件名原样复制到新项目，除非用户明确要求保留。
- 当 `ui-design.md` 与用户显式输入冲突时，以用户显式输入为准，并在生成结果中保留差异说明或 `待确认`。
- 当用户提供 `ui-design.md` 且项目包含 UI 端时，`rules/ui-design.md` 必须优先基于该文档生成。
- 当用户提供 `ui-design.md` 但产品形态未启用 UI 端时，应询问是否启用 UI；若不启用，只登记为参考资料，不生成强制 UI 规则。

### 治理流程与默认命令

Harness 默认包含五套 Agent 工具目录：`.cursor/`、`.claude/`、`.codex/`、`.kiro/`、`.opencode/`。初始化时默认保留同一套命令语义。

| 流程 | 变量 | 默认 | 命令 |
|---|---|---|---|
| 需求治理 | `REQ_ROOT_DIR`、`REQ_ID_PATTERN`、`REQ_STATUS_MACHINE` | 启用 | `/req-capture`、`/req-explore`、`/req-generate`、`/req-complete`、`/req-review`、`/req-opsx` |
| 缺陷治理 | `BUG_ROOT_DIR`、`BUG_ID_PATTERN`、`BUG_STATUS_MACHINE` | 启用 | `/bug-capture`、`/bug-explore`、`/bug-generate`、`/bug-complete`、`/bug-review`、`/bug-opsx` |
| Sprint 治理 | `SPRINT_ROOT_DIR`、`ITERATION_PATTERN` | 启用，可标记 planned | `/sprint-propose`、`/sprint-explore`、`/sprint-apply`、`/sprint-archive` |
| OpenSpec | `OPENSPEC_ENABLED`、`CHANGE_ID_PATTERN`、`SPEC_MODULES` | 默认启用 | `/opsx-explore`、`/opsx-propose`、`/opsx-apply`、`/opsx-archive` |
| 项目基线 | `PROJECT_GOVERNANCE_LEVEL` | 启用 | `/initialize-project`、`/build-design-system`、`/build-api-standard`、`/build-test-framework` |

### 文档与标准模块

| 模块 | 变量 | 默认 | 影响文件 |
|---|---|---|---|
| 文档治理 | `DOCUMENT_SOURCE_POLICY`、`DOCUMENT_REVIEW_POLICY`、`DOCUMENT_UPDATE_POLICY` | AI 生成 + 人工 Review | DOCUMENT_METADATA_INDEX、docs/README、rules/document-governance |
| API 标准 | `API_ENABLED`、`OPENAPI_ENABLED`、`API_PREFIX`、`RESPONSE_ENVELOPE` | 有 API 时启用 | docs/03、standards/api/openapi/error/auth |
| 认证授权 | `AUTH_ENABLED`、`AUTH_STRATEGY`、`PERMISSION_MODEL` | 有用户/权限时启用 | security、authentication |
| 文件上传 | `UPLOAD_ENABLED`、`UPLOAD_METHOD`、`UPLOAD_ALLOWED_TYPES` | 有上传时启用 | file_upload、media、object-storage |
| 测试体系 | `TEST_FRAMEWORKS`、`TEST_COMMANDS`、`CI_TEST_GATE` | 默认启用，未知待确认 | testing、testing-governance、unit/frontend/coverage |
| 覆盖率 | `COVERAGE_ENABLED`、`COVERAGE_TARGET`、`COVERAGE_TOOLING` | 可默认 planned | test-coverage |
| UI 设计 | `UI_STACK`、`DESIGN_SYSTEM_ENABLED`、`DESIGN_TOKEN_SOURCE` | 有 UI 时启用 | ui-design、build-design-system |
| 对象存储策略 | `OBJECT_STORAGE_MODE`、`BUCKET_NAMING_RULE`、`OBJECT_KEY_PATTERN` | 有对象存储时启用 | object-storage、docs/07 |

### 部署、环境与目录

| 参数 | 变量 | 默认/处理方式 |
|---|---|---|
| 环境列表 | `ENVIRONMENTS`、`STAGING_ENABLED`、`PRODUCTION_ENABLED` | local 默认启用，其他默认 false |
| Docker Compose | `DOCKER_COMPOSE_ENABLED`、`DOCKER_UP_COMMAND`、`DOCKER_DOWN_COMMAND` | 部署方式含 docker-compose 时启用 |
| Kubernetes | `KUBERNETES_ENABLED` | 默认 false |
| 服务矩阵 | `SERVICE_MATRIX`、`SERVICE_NAME`、`SERVICE_TYPE`、`SERVICE_INTERNAL_PORT` | 根据能力生成，未知待确认 |
| 端口策略 | `PORT_STRATEGY`、`PORT_ENV_VARS` | 默认“容器内固定，宿主机可配置” |
| 环境变量策略 | `PUBLIC_ENV_PREFIX`、`SECRET_POLICY` | 未提供则使用模板默认规则 |
| 本地运行时数据 | `RUNTIME_DATA_PATH`、`SEED_DATA_POLICY`、`FIXTURE_POLICY` | 默认 `data/`，不提交运行时数据 |
| 模型资产 | `MODEL_ASSET_POLICY`、`MODEL_PATHS` | 算法/模型启用时使用 `models/` |
| 部署配置 | `DEPLOYMENT_OWNER`、`PRIMARY_DEPLOY_COMMAND` | 未提供则 `待确认` |

### 测试与验证

| 参数 | 变量 | 默认/处理方式 |
|---|---|---|
| 主要验证命令 | `PRIMARY_VERIFY_COMMAND` | 默认 `待确认`；如脚本存在可用 `./scripts/run-tests.sh` |
| 目录校验命令 | `DIRECTORY_VALIDATE_COMMAND` | 默认 `python scripts/validate-directory-structure.py` |
| OpenSpec 校验 | `OPENSPEC_VALIDATE_COMMAND` | 默认 `./scripts/validate-openspec.sh` 或 `待确认` |
| 单元测试命令 | `UNIT_TEST_COMMAND` | 未提供则 `待确认` |
| 集成测试命令 | `INTEGRATION_TEST_COMMAND` | 未提供则 `待确认` |
| 前端测试命令 | `FRONTEND_TEST_COMMAND` | 无前端则 `不适用` |
| E2E 测试命令 | `E2E_TEST_COMMAND` | 未启用则 `不适用` |
| 覆盖率命令 | `COVERAGE_COMMAND` | 未启用则 `planned` 或 `不适用` |
| 测试证据策略 | `EVIDENCE_STORAGE_POLICY` | 默认登记 CI URL/报告路径，不提交临时产物 |

### 输入缺省规则

| 缺失信息 | 处理规则 |
|---|---|
| 业务、角色、能力不明确 | 写 `待确认`，保留结构，不编造业务 |
| 技术栈不明确 | 写 `待确认`，删除对应强制命令 |
| 能力未启用 | 设置 `enabled=false` 或删除强制章节、服务和目录 |
| 命令未知 | 写 `待确认`，不得生成不可执行命令 |
| 密钥、密码、Token 未提供 | 只生成变量名，不生成真实值 |
| OpenSpec 未启用 | 保留最小说明或指向等价变更系统 |
| 测试未启用 | 保留治理目标和未来启用计划，删除强制门禁 |
| 提供 `ui-design.md` 但部分设计信息缺失 | 已解析内容写入对应变量，缺失项标记 `待确认`，不补造品牌、页面或 Token |
...
5. 目标用户与核心能力
6. 后端、前端、数据库、对象存储、部署方式
7. 是否启用认证、上传/媒体、算法/模型、OpenSpec、测试覆盖率
8. 主要启动、测试、部署命令
9. 是否有可选输入文档，例如现有 `ui-design.md`

选填项不阻塞流程；缺失信息按“输入缺省规则”处理。

如果用户提供 `ui-design.md`：
1. 读取用户粘贴内容或指定文件路径。
2. 提取设计定位、目标用户、支持端、前端/UI 技术栈、组件库、图标库、Token 来源、核心页面、核心组件、响应式、可访问性、主题、国际化、视觉验收和 UI 验证命令。
3. 将提取结果写入 Step 2 的 UI 派生变量。
4. 在文件生成阶段优先用于 `rules/ui-design.md`，并同步影响相关 README、project.yaml、docs 和 frontend-test 标准。
...
```
PRODUCT_NAME        = 用户输入的产品名称
PRODUCT_CODE        = 用户输入或由产品名称派生的英文 slug，小写 kebab-case
PRODUCT_DESCRIPTION = 产品简介
BUSINESS_DOMAIN     = 业务领域，未知则待确认
TARGET_USERS        = 目标用户，未知则待确认
CORE_CAPABILITIES   = 核心能力，未知则待确认
PRODUCT_FORMS       = 产品形态列表

HAS_BACKEND         = PRODUCT_FORMS 包含 API/后端/服务端，或用户明确启用
HAS_API_SERVICE     = PRODUCT_FORMS 包含 API 服务
HAS_FRONTEND        = PRODUCT_FORMS 包含任一 UI 端
HAS_WEB             = PRODUCT_FORMS 包含 Web/管理后台/H5
HAS_WEB_PUBLIC      = PRODUCT_FORMS 包含 Web 展示端
HAS_WEB_ADMIN       = PRODUCT_FORMS 包含管理后台
HAS_MINIAPP         = PRODUCT_FORMS 包含小程序
HAS_MOBILE          = PRODUCT_FORMS 包含 Android/iOS/移动端
HAS_DESKTOP         = PRODUCT_FORMS 包含桌面端
HAS_SDK             = PRODUCT_FORMS 包含 SDK
HAS_ALGORITHM       = PRODUCT_FORMS 包含算法/模型服务

API_ENABLED             = HAS_API_SERVICE 或 HAS_BACKEND
FRONTEND_ENABLED        = HAS_FRONTEND
AUTH_ENABLED            = 用户启用认证/权限/登录，否则 false 或待确认
MEDIA_ENABLED           = 用户启用媒体/上传/导入导出，否则 false
UPLOAD_ENABLED          = MEDIA_ENABLED 或用户启用上传
OBJECT_STORAGE_ENABLED  = 用户启用对象存储，或 MEDIA_ENABLED=true
ASYNC_TASK_ENABLED      = 用户启用异步任务，否则 false
AI_OR_ALGORITHM_ENABLED = HAS_ALGORITHM

BACKEND_STACK       = 后端技术栈描述，未知则待确认
FRONTEND_STACK      = 前端技术栈描述，未启用则不适用
DATABASE_STACK      = 数据库选型，默认 SQLite 但标记可确认
OBJECT_STORAGE_STACK = 对象存储选型，未启用则不适用
DEPLOYMENT_STACK    = 部署方式，默认 docker-compose

REQ_ROOT_DIR        = issues/requirements
BUG_ROOT_DIR        = issues/bugs
SPRINT_ROOT_DIR     = iterations
CHANGE_ROOT_DIR     = openspec/changes
SPEC_ROOT_DIR       = openspec/specs
TEST_ROOT_DIR       = tests

OPENSPEC_ENABLED    = 默认 true，除非用户明确禁用
DOCKER_COMPOSE_ENABLED = DEPLOYMENT_STACK 包含 docker-compose
KUBERNETES_ENABLED  = DEPLOYMENT_STACK 包含 kubernetes/helm

DIRECTORY_VALIDATE_COMMAND = python scripts/validate-directory-structure.py
OPENSPEC_VALIDATE_COMMAND  = ./scripts/validate-openspec.sh 或待确认
PRIMARY_VERIFY_COMMAND     = 用户输入；否则待确认

UI_DESIGN_SOURCE_ENABLED = 用户提供 ui-design.md 时 true，否则 false
UI_DESIGN_SOURCE_PATH    = 用户提供的 ui-design.md 路径；粘贴内容时标记为 pasted-input
UI_DESIGN_SOURCE_CONTENT = ui-design.md 原文内容，仅用于解析，不直接复制到输出
UI_STACK                 = 从 ui-design.md 或用户输入提取；未知则待确认
DESIGN_STYLE             = 从 ui-design.md 提取设计风格/视觉方向；未知则待确认
DESIGN_PRINCIPLES        = 从 ui-design.md 提取设计原则；未知则待确认
DESIGN_TOKEN_SOURCE      = 从 ui-design.md 提取 Token 来源；未知则待完善
COMPONENT_LIBRARY        = 从 ui-design.md 或前端技术栈提取；未知则待确认
ICON_LIBRARY             = 从 ui-design.md 提取；未知则待确认
ROUTE_LAYOUTS            = 从 ui-design.md 提取页面布局；未知则待确认
CORE_PAGES               = 从 ui-design.md 提取核心页面；未知则待确认
CORE_COMPONENTS          = 从 ui-design.md 提取核心组件；未知则待确认
RESPONSIVE_POLICY        = 从 ui-design.md 提取响应式规则；未知则待确认
ACCESSIBILITY_TARGET     = 从 ui-design.md 提取可访问性目标；未知则待确认
I18N_POLICY              = 从 ui-design.md 提取国际化规则；未知则待确认
THEMING_POLICY           = 从 ui-design.md 提取主题/暗色/白标规则；未知则待确认
VISUAL_ACCEPTANCE_SOURCE = 从 ui-design.md 提取视觉验收来源；未知则待确认
UI_VERIFY_COMMANDS       = 从 ui-design.md、package.json 或脚本提取；未知则待确认
```
...
```
{PRODUCT_CODE}/
├── .claude/                    # 保留默认 commands + skills
├── .codex/                     # 保留默认 prompts + skills
├── .cursor/                    # 保留默认 commands + skills
├── .kiro/                      # 保留默认 prompts + skills
├── .opencode/                  # 保留默认 commands + skills
├── AGENTS.md                   # 必须更新
├── README.md                   # 必须更新
├── project.yaml                # 必须更新
...
- `.cursor/`、`.claude/`、`.codex/`、`.kiro/`、`.opencode/` 必须保留同一套默认命令语义；不同工具只允许按目录约定使用 commands、prompts 或分组路径。
...
- 保留默认需求命令族 `/req-capture`、`/req-explore`、`/req-generate`、`/req-complete`、`/req-review`、`/req-opsx`，并确保五套 Agent 工具目录语义一致。
...
- 保留默认缺陷命令族 `/bug-capture`、`/bug-explore`、`/bug-generate`、`/bug-complete`、`/bug-review`、`/bug-opsx`，并确保五套 Agent 工具目录语义一致。
...
- 解析 `ui-design.md` 时，用户显式输入优先级高于文档内容；冲突项必须标记 `待确认` 或写入生成说明，不得静默覆盖。
...
基于 `assets/pm-harness/README.md` 的模块化框架生成，作为工程根入口，覆盖产品简介、用户角色、核心能力、技术栈、快速启动、部署入口、目录导航、AI 约束、文档索引和初始化建议。
...
- 根据是否启用 Docker、Kubernetes、对象存储、媒体、算法/模型、移动端、小程序、桌面端、OpenSpec、Sprint 治理保留或删除 `[条件启用]` 内容。
...
基于 `assets/pm-harness/docker-compose.yml` 的模块化框架生成，作为本地开发、演示部署和服务拓扑的 Compose 入口。
...
基于 `assets/pm-harness/DOCUMENT_METADATA_INDEX.md` 的模块化框架生成，作为文档资产、配置资产和脚本资产的元数据登记入口。
...
- 所有产品名、项目代码替换
- `README.md`：基于 `assets/pm-harness/docs/README.md` 的模块化框架生成，作为 `docs/` 目录总入口、主文档阅读顺序、专项治理文档、知识库和文档边界说明。
...
- `docs/standards/api-governance.md`：基于 `assets/pm-harness/docs/standards/api-governance.md` 的模块化框架生成，作为 API 设计原则、资源命名、URL/Method/版本、请求响应、错误码、认证授权、分页排序、幂等、OpenAPI、客户端生成、测试和维护规则入口。
...
- `docs/standards/openapi-rules.md`：基于 `assets/pm-harness/docs/standards/openapi-rules.md` 的模块化框架生成，作为 OpenAPI 契约来源、路由元数据、Schema、响应、错误码、安全声明、Tags、客户端生成、校验和兼容规则入口。
...
- `docs/standards/authentication.md`：基于 `assets/pm-harness/docs/standards/authentication.md` 的模块化框架生成，作为认证方式、登录登出、Token/Session、权限模型、受保护接口、前端登录态、错误码、安全测试和维护规则入口。
...
- `docs/standards/error-codes.md`：基于 `assets/pm-harness/docs/standards/error-codes.md` 的模块化框架生成，作为错误码分段、HTTP 映射、响应结构、登记流程、实现同步、前端处理、测试和维护规则入口。
...
- `docs/standards/file_upload.md`：基于 `assets/pm-harness/docs/standards/file_upload.md` 的模块化框架生成，作为文件上传入口、认证授权、格式大小限制、安全校验、对象存储、元数据、响应结构、错误码、测试和维护规则入口。
...
- `docs/standards/testing-governance.md`：基于 `assets/pm-harness/docs/standards/testing-governance.md` 的模块化框架生成，作为测试目标、测试分层、目录职责、测试类型策略、AI 补测要求、CI 门禁、覆盖率治理、测试数据和例外审批的测试体系总纲。
  ...
  - 根据 BACKEND_STACK、FRONTEND_STACK、DATABASE_STACK、OBJECT_STORAGE_STACK、PRODUCT_FORMS、HAS_WEB、HAS_MINIAPP、HAS_MOBILE、HAS_SDK、HAS_ALGORITHM 生成测试类型、目录职责和分层策略；未启用能力不得保留强制测试要求。
...
- `docs/standards/unit-test-standard.md`：基于 `assets/pm-harness/docs/standards/unit-test-standard.md` 的模块化框架生成，作为单元测试范围、目录命名、断言质量、Mock 边界、测试数据、覆盖要求、AI 补测规则和维护规范入口。
...
- `docs/standards/frontend-test-standard.md`：基于 `assets/pm-harness/docs/standards/frontend-test-standard.md` 的模块化框架生成，作为前端测试栈、组件测试、页面测试、API Mock、可访问性、视觉回归、E2E 分工、运行命令和 CI 门禁规则入口。
  ...
  - 当 FRONTEND_ENABLED=false 且项目无 Web、管理后台、H5、桌面端、小程序、移动 Web 或可视化界面时，只保留启用条件和未来启用说明，删除强制测试栈、目录、组件测试和 E2E 要求。
  - 根据 FRONTEND_STACK 和 FRONTEND_TEST_STACK 生成 React、Vue、Svelte、Angular、小程序、桌面端或其他前端栈的测试工具和目录约定；不得保留未启用框架的强制测试工具。
...
- `docs/standards/test-coverage.md`：基于 `assets/pm-harness/docs/standards/test-coverage.md` 的模块化框架生成，作为覆盖率目标、分层门槛、统计范围、排除规则、测量命令、CI 门禁、例外审批和趋势治理规则入口。
  ...
  - 根据 BACKEND_STACK、FRONTEND_STACK、MOBILE_STACK、HAS_WEB、HAS_MINIAPP、HAS_SDK、HAS_ALGORITHM、HAS_SCRIPTS 生成分层覆盖率目标；未启用端或模块不得保留强制覆盖率门槛。
...
- `00-product-overview.md`：基于 `assets/pm-harness/docs/00-product-overview.md` 的模块化框架生成，作为产品定位、目标用户、用户痛点、产品形态、核心场景、核心能力、范围边界、数据对象、成功指标和文档导航入口。
  ...
  - 根据项目能力保留或删除 `[条件启用]` 模块，例如 Web、管理后台、小程序、移动端、桌面端、API/SDK、媒体、对象存储、算法/模型服务。
...
- `01-architecture.md`：基于 `assets/pm-harness/docs/01-architecture.md` 的模块化框架生成，作为系统上下文、模块职责、源码分层、运行时拓扑、关键数据流、存储架构、API/集成、安全权限、非功能要求、AI 开发边界和架构决策入口。
  ...
  - 根据项目能力保留或删除 `[条件启用]` 模块，例如后端、Web、小程序、移动端、共享层、SDK、对象存储、媒体、异步任务、算法/模型、外部集成、缓存/搜索、Kubernetes/私有化部署。
...
- `02-deployment.md`：基于 `assets/pm-harness/docs/02-deployment.md` 的模块化框架生成，作为部署目标、服务组成、环境变量、端口策略、运行方式、数据持久化、安全要求、验证与回滚入口。
...
- `03-api-index.md`：基于 `assets/pm-harness/docs/03-api-index.md` 的模块化框架生成，作为 API 分组、基础约定、认证授权、响应结构、错误码、OpenAPI、客户端生成、接口维护规则入口。
...
- `04-database-design.md`：基于 `assets/pm-harness/docs/04-database-design.md` 的模块化框架生成，作为数据库选型、Schema 来源、表清单、字段规范、关系约束、索引、迁移、种子数据、数据安全和维护规则入口。
...
- `05-compatibility-matrix.md`：基于 `assets/pm-harness/docs/05-compatibility-matrix.md` 的模块化框架生成，作为产品形态兼容、能力覆盖、浏览器与设备、运行时、数据库、对象存储、部署环境、操作系统、CPU 架构、API 兼容和验证规则入口。
  ...
  - 根据 PRODUCT_FORMS 保留或删除 Web、管理后台、小程序、移动端、桌面端、API/SDK 等端能力列；未启用端不得出现在强制矩阵中。
  - 根据项目能力保留或删除 `[条件启用]` 模块，例如浏览器兼容、移动端/小程序/桌面端、数据库、对象存储、外部服务、信创、私有化部署、OS/CPU。
...
- `06-video-asset-management.md`：基于 `assets/pm-harness/docs/06-video-asset-management.md` 的模块化框架生成，作为视频与富媒体资产场景、端能力、上传、对象存储、封面、转码、播放、元数据、安全、测试和验收规则入口。
  ...
  - 根据 PRODUCT_FORMS 保留或删除 Web、管理后台、移动端、小程序、桌面端、API/SDK 的端能力矩阵；未启用端不得出现在强制能力中。
  - 根据项目能力保留或删除 `[条件启用]` 模块，例如对象存储、封面、转码、CDN、API、前端体验、移动端/小程序适配、内容审核。
...
- `07-object-storage-strategy.md`：基于 `assets/pm-harness/docs/07-object-storage-strategy.md` 的模块化框架生成，作为对象存储选型、桶/容器策略、资源前缀、对象 key、访问控制、上传下载、元数据、生命周期、备份迁移、成本治理和测试验收规则入口。
...
- `openspec/project.md`：基于 `assets/pm-harness/openspec/project.md` 的模块化框架生成，作为 OpenSpec 项目上下文，覆盖项目背景、产品范围、技术栈、目录职责、研发事实源、Change ID、规格模块、执行流程、AI 约束、测试映射和文档同步关系。
...
- `openspec/config.yaml`：基于 `assets/pm-harness/openspec/config.yaml` 的模块化 YAML 配置生成，作为 OpenSpec 项目标识、目录路径、语言策略、Change 命名、规格策略、校验命令、归档策略和 AI guardrails 配置入口。
  - 保留顶部注释元数据，包括 purpose、content、source、update_method、owner、note；不得给 YAML 文件插入 Markdown frontmatter。
...
- `openspec/testing-mapping.md`：基于 `assets/pm-harness/openspec/testing-mapping.md` 的模块化框架生成，作为 Requirement、Bug、OpenSpec Change、验收项、测试用例、验证命令、测试证据和例外审批的追溯索引。
...
`.cursor/`、`.claude/`、`.codex/`、`.kiro/`、`.opencode/` 默认保留同一套自定义命令语义。初始化时必须保留以下命令族，并对命令内容中的产品名、路径、示例业务和技术栈做通用化替换：

| 命令族 | 默认命令 | 生成代码 |
|---|---|---|
| 需求治理 | `/req-capture`、`/req-explore`、`/req-generate`、`/req-complete`、`/req-review`、`/req-opsx` | 否 |
| 缺陷治理 | `/bug-capture`、`/bug-explore`、`/bug-generate`、`/bug-complete`、`/bug-review`、`/bug-opsx` | 否 |
| Sprint 治理 | `/sprint-propose`、`/sprint-explore`、`/sprint-apply`、`/sprint-archive` | 仅 `/sprint-apply` 会编排开发 |
| 项目基线 | `/initialize-project`、`/build-design-system`、`/build-api-standard`、`/build-test-framework` | 以生成治理文档和基线为主 |
| OpenSpec | `/opsx-explore`、`/opsx-propose`、`/opsx-apply`、`/opsx-archive` | 仅 `/opsx-apply` 进入实现 |

不同工具只允许因目录约定调整文件路径，例如 `.claude/commands/{group}/*.md`、`.codex/prompts/*.md`、`.kiro/prompts/*.prompt.md`，不得改变同名命令的阶段、输入、输出、是否生成文档和是否生成代码边界。
