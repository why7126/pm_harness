...
- 所有输入项都必须向用户提示填写，包括推荐输入、能力开关、技术栈、Agent 工具、治理流程、部署环境、验证命令和可选输入文档；不得因为字段非必填就完全不提示。
- 非必填输入项可分批提示，用户跳过或未回答时不阻塞生成，但必须按“输入缺省规则”记录为 `待确认`、`enabled=false`、`planned`、`不适用` 或使用明确的默认值。
...
必须提示用户选择本项目使用的 Agent 工具。该项非必填，但必须询问；用户未选择时默认生成全部支持目录，并在文档中标记为“默认启用，待项目确认”。

| 参数 | 变量 | 默认/处理方式 | 影响范围 |
|---|---|---|---|
| 使用的 Agent 工具 | `ENABLED_AGENT_TOOLS` | 未提供默认 `Claude Code, Cursor, Codex, OpenCode, Kiro`，并标记待确认 | `.claude/`、`.cursor/`、`.codex/`、`.opencode/`、`.kiro/`、AGENTS、rules/global、rules/directory-structure |
| 主要 Agent 工具 | `PRIMARY_AGENT_TOOL` | 未提供默认 `Claude Code`，并标记待确认 | README、AGENTS、常用命令说明 |
| 是否生成多 Agent 目录 | `MULTI_AGENT_ENABLED` | `ENABLED_AGENT_TOOLS` 超过 1 个时 true | Agent 工具目录、同步规则、命令一致性检查 |
| Agent 命令同步策略 | `AGENT_COMMAND_SYNC_POLICY` | 默认“以 `.claude/` 语义为事实源，生成其它工具目录” | `.cursor/`、`.codex/`、`.opencode/`、`.kiro/` |
...
Harness 支持五套 Agent 工具目录：`.cursor/`、`.claude/`、`.codex/`、`.kiro/`、`.opencode/`。初始化时必须根据 `ENABLED_AGENT_TOOLS` 生成对应目录；用户未选择时默认生成五套目录，并保留同一套命令语义。
...
| Agent 工具未选择 | 默认 `Claude Code, Cursor, Codex, OpenCode, Kiro`，标记为“默认启用，待项目确认” |
...
所有非必填项也必须提示用户填写，可分批询问；用户跳过时不阻塞生成：

5. 目标用户与核心能力
6. 后端、前端、数据库、对象存储、部署方式
7. 使用的 Agent 工具：Claude Code、Cursor、Codex、OpenCode、Kiro，可多选
8. 主要 Agent 工具和是否需要多 Agent 目录
9. 是否启用认证、上传/媒体、算法/模型、OpenSpec、测试覆盖率
10. 主要启动、测试、部署命令
11. 是否有可选输入文档，例如现有 `ui-design.md`
...
ENABLED_AGENT_TOOLS = 用户选择的 Agent 工具；未提供时默认 Claude Code, Cursor, Codex, OpenCode, Kiro，并标记待确认
PRIMARY_AGENT_TOOL  = 用户选择的主要 Agent；未提供时默认 Claude Code，并标记待确认
MULTI_AGENT_ENABLED = ENABLED_AGENT_TOOLS 数量大于 1
AGENT_COMMAND_SYNC_POLICY = 默认以 .claude/ 命令语义为事实源生成其它 Agent 工具目录
...
│   └── knowledge-base/         # 初始化阶段创建，置空
├── issues/
│   ├── requirements/
│   │   └── template/           # 需求模板目录（详见文件规则）
│   └── bugs/
│       └── template/           # Bug模板目录（详见文件规则）
├── iterations/                 # 初始化阶段创建，置空
├── openspec/
│   ├── project.md
│   ├── config.yaml
│   ├── specs/                  # 初始化阶段创建，置空
...
基于 `assets/templates/requirements-template-README.md` 和 `assets/templates/requirement-template.md` 创建如下结构（帮助用户了解如何创建需求）：
...
`assets/pm-harness-template/` 仅预置 `.claude/` 作为命令和 OpenSpec Skill 的规范样本。初始化时必须基于 `.claude/` 样本和下方命令清单，为 `ENABLED_AGENT_TOOLS` 中启用的工具生成对应目录；用户未选择时默认生成五套工具目录。所有已启用工具必须保留同一套自定义命令语义，并对命令内容中的产品名、路径、示例业务和技术栈做通用化替换：
...
生成映射：
- `Claude Code` 启用时，`.claude/commands/{group}/*.md` 直接使用模板样本，按项目参数更新内容。
- `Claude Code` 启用时，`.claude/skills/openspec-*/SKILL.md` 基于模板样本 `SKILL.template.md` 生成，按项目参数更新内容。
- `Cursor` 启用时，`.cursor/commands/*.md` 基于 `.claude/commands/` 的同名命令语义生成，文件名使用扁平命令名，例如 `req-capture.md`、`opsx-apply.md`。
- `Cursor` 启用时，`.cursor/skills/openspec-*/SKILL.md` 基于 `.claude/skills/*/SKILL.template.md` 生成。
- `Codex` 启用时，`.codex/prompts/*.md` 基于 `.claude/commands/` 的同名命令语义生成，文件名使用扁平命令名。
- `Codex` 启用时，`.codex/skills/openspec-*/SKILL.md` 基于 `.claude/skills/*/SKILL.template.md` 生成。
- `Kiro` 启用时，`.kiro/prompts/*.prompt.md` 基于 `.claude/commands/` 的同名命令语义生成，文件名使用扁平命令名并追加 `.prompt.md`。
- `Kiro` 启用时，`.kiro/skills/openspec-*/SKILL.md` 基于 `.claude/skills/*/SKILL.template.md` 生成。
- `OpenCode` 启用时，`.opencode/commands/*.md` 基于 `.claude/commands/` 的同名命令语义生成，文件名使用扁平命令名。
- `OpenCode` 启用时，`.opencode/skills/openspec-*/SKILL.md` 基于 `.claude/skills/*/SKILL.template.md` 生成。
- 如果 `Claude Code` 未启用，`.claude/` 仅作为生成过程中的临时事实源，最终输出不得保留。

不同工具只允许因目录约定调整文件路径、文件后缀和 frontmatter，不得改变同名命令的阶段、输入、输出、是否生成文档和是否生成代码边界。
