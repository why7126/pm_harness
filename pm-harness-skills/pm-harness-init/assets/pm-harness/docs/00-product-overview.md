...
本文不替代详细需求文档。具体需求应沉淀到 `issues/requirements/REQ-*`，具体 BUG 应沉淀到 `issues/bugs/BUG-*`，架构、接口、数据库、部署、兼容性等长期规则应进入对应 `docs/` 与 `rules/` 文件。
...
| 小程序 | `{MINIAPP_USERS}` | `{MINIAPP_RESPONSIBILITY}` | `{MINIAPP_ENTRY}` | `{MINIAPP_STATUS}` |
...
| `issues/requirements/` | 需求记录和需求包 |
| `issues/bugs/` | BUG 记录和缺陷包 |
...
4. 未启用 Web、小程序、移动端、媒体、对象存储、算法模型时，删除对应产品形态和能力说明。
5. 对缺失但重要的信息标记 `待确认`。
