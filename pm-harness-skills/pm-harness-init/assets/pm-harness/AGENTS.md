...
| `{PRODUCT_FORMS}` | 产品形态，如 Web、小程序、移动端、桌面端、API 服务 | `[个性化]` |
...
| UI 与 Design System | 否 | `[条件启用]` | 有前端、移动端、小程序或桌面 UI 时启用 |
...
22. 当前任务对应的 issues/、iterations/ 或 openspec/changes/<change-id>/
```
...
| 任务类型 | 必须额外读取 |
|---|---|
| 需求治理 | `rules/requirement-management.md`、对应 `issues/requirements/REQ-*`、相关 `iterations/` |
| BUG 治理 | `rules/bug-management.md`、对应 `issues/bugs/BUG-*`、相关日志、截图和回归记录 |
...
| `rules/ui-design.md` | 控制 Web、移动端、小程序、桌面端 UI 一致性 | `[条件启用]` |
...
| 小程序 | `src/miniapp/` | 小程序页面与端能力 | `src/miniapp/README.md` | `{MINIAPP_STACK}` |
...
| `issues/requirements/` | 需求管理 | 每个需求独立目录 |
| `issues/bugs/` | BUG 管理 | 每个 BUG 独立目录 |
| `iterations/` | 迭代管理 | sprint 目标、范围、验收、发布说明 |
...
```text
issues/requirements/REQ-xxxx-name/
```

如需要 BUG 记录，放入：

```text
issues/bugs/BUG-xxxx-name/
```
...
- 同步生成或更新客户端类型。
- 补充契约测试、集成测试或兼容性验证。
- 在回复中说明是否影响前端、后端、SDK、移动端、小程序或第三方集成。
