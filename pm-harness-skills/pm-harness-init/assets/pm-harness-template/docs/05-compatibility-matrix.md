...
> - **[条件启用]**：只有项目具备对应能力时才保留或展开，例如 Web、小程序、移动端、桌面端、数据库、对象存储、信创、私有化部署、算法服务。
...
| `{PRODUCT_FORMS}` | 产品形态，如 Web、管理后台、小程序、移动端、桌面端、API 服务 | 待确认 |
...
| 能力 | Web | 管理后台 | 小程序 | 移动端 | 桌面端 | API/SDK | 说明 |
|---|---|---|---|---|---|---|---|
| `{CAPABILITY_1}` | `{WEB_SUPPORT_1}` | `{ADMIN_SUPPORT_1}` | `{MINIAPP_SUPPORT_1}` | `{MOBILE_SUPPORT_1}` | `{DESKTOP_SUPPORT_1}` | `{API_SUPPORT_1}` | `{NOTE_1}` |
| `{CAPABILITY_2}` | `{WEB_SUPPORT_2}` | `{ADMIN_SUPPORT_2}` | `{MINIAPP_SUPPORT_2}` | `{MOBILE_SUPPORT_2}` | `{DESKTOP_SUPPORT_2}` | `{API_SUPPORT_2}` | `{NOTE_2}` |
...
## 5. 移动端 / 小程序 / 桌面端兼容 `[条件启用]`

| 客户端 | 平台/系统 | 最低版本 | 支持等级 | 验证方式 | 备注 |
|---|---|---:|---|---|---|
| 小程序 | `{MINIAPP_PLATFORM}` | `{MINIAPP_MIN_VERSION}` | `{MINIAPP_SUPPORT}` | `{MINIAPP_VERIFY}` | `{MINIAPP_NOTE}` |
...
- 新增或移除产品形态。
- 新增核心能力或改变不同端支持范围。
- 升级浏览器、移动端、小程序、桌面端、运行时、框架、数据库或对象存储要求。
...
5. 根据是否启用 Web、小程序、移动端、桌面端、对象存储、外部服务、信创或私有化部署保留对应 `[条件启用]` 模块。
6. 未确认信息标记为 `待确认`。
