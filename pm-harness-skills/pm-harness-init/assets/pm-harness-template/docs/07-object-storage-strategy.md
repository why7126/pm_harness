...
| `{OBJECT_KEY_PATTERN}` | 对象 key 命名规则 | `{prefix}/default/{resource_type}/{uuid}.{ext}` |
...
key: {prefix}/default/{resource_type}/{uuid}.{ext}
```
...
| `images/` | 图片 | 图片资源 | 按需 |
| `videos/` | 视频 | 视频资源 | 按需 |
| `audios/` | 音频 | 音频、录音、语音样本 | 按需 |
| `files/` | 文件 | 文档、导入、导出、模型、附件等通用文件 | 按需 |
| `tmp/` | 临时文件 | 上传、导入、处理中的短期文件 | 按需 |

前缀要求：

- 前缀必须反映资源大类，不得直接使用临时业务命名或生命周期命名。
- 前缀必须是资源大类，例如 `images/`、`videos/`、`files/`、`audios/`；不得使用 `original/`、`processed/`、`thumbnails/` 作为顶层前缀。
- 临时文件必须有过期清理策略。
...
| `resource_type` | 业务资源路径，例如 `user/avatars`、`brands/logos`、`products/videos`、`imports/source` |
